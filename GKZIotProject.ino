...
const unsigned long LOG_INTERVAL_MS   = 60000;  // append one row every 60s

// Cloud relay (optional - for access from anywhere)

const bool  CLOUD_ENABLED = false;
const char* CLOUD_HOST    = "https://your-service.onrender.com";
const char* CLOUD_API_KEY = "changeme";
const unsigned long CLOUD_PUSH_INTERVAL_MS = 60000; // push once a minute, same cadence as local logging
const unsigned long SAMPLE_INTERVAL_MS = 2000;  // refresh the API's live cache every 2s
int historyRowCount = 0; 
bool fsOK = false; // ADDED: true once LittleFS mounts successfully

const float MQ7_RL_OHMS  = 10000.0;  
const float MQ7_RO_OHMS  = 10000.0;  // PLACEHOLDER clean-air resistance - to be calibrated
const float MQ7_CURVE_M  = -1.5;     // PLACEHOLDER log-log slope
const float MQ7_CURVE_B  = 0.53;     // PLACEHOLDER log-log intercept
...
// ---------- Cloud relay push ----------
// POSTs the current reading to cloud-server/app.py, if CLOUD_ENABLED.
// This is an outbound-only request - no inbound port forwarding or
// firewall changes needed on your home network for this to work.
void pushToCloud() {
  if (!CLOUD_ENABLED || WiFi.status() != WL_CONNECTED) return;

  WiFiClientSecure client;
  client.setInsecure(); // skip certificate validation - simplest for a hobby project;
                         // for stricter security this could be replaced with the host's
                         // real CA certificate via client.setCACert(...)
  HTTPClient https;
  String url = String(CLOUD_HOST) + "/api/ingest";

  if (!https.begin(client, url)) {
    Serial.println("[CLOUD] Could not begin HTTPS connection");
    return;
  }
  ...
  snprintf(json, sizeof(json),
    "{\"ts\":\"%s\",\"tempC\":%.2f,\"humidity\":%.2f,\"pressureHpa\":%.2f,"
    "\"coPpm\":%.2f,\"voltageV\":%.3f,\"vibrationG\":%.3f,\"uptimeS\":%lu}",
    ts, liveData.tempC, liveData.humidity, liveData.pressureHpa,
    liveData.coPpmEstimate, liveData.gasVoltage, liveData.vibrationG, liveData.uptimeS);
...
void setupWebServer() {
  ...
  // ADDED: serve the dashboard's own files straight from flash, so
  // visiting the ESP32's own address (e.g. http://192.168.1.50/) loads
  // the dashboard same-origin with the API - no IP to configure, no
  // CORS, no HTTPS/HTTP mixed-content issues. Requires uploading the
  // "data" folder to LittleFS once via the IDE's filesystem uploader
...
void loop() {
  ...
  if (CLOUD_ENABLED && now - lastCloudPush >= CLOUD_PUSH_INTERVAL_MS) {
    lastCloudPush = now;
    pushToCloud();
  }
